...
I will assume Employees and Bosses are stored together in the Employee Table.
...
The code returns 6 instead of returning values 1 to 5 periodically. 
It's not working as expected because the for loop is synchronuous, and the setTimeout method is not wrapped asynchronously. 

-- 2. Rewrite the code to produce the expected output (important: we need to keep the 1000ms interval!)
Fix the timing issue by using things like callbacks, async/await or chaining promises.
function counter() {
 var i = 1;
